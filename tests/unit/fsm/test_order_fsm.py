...
def make_fsm(initial_state: OrderState) -> tuple[OrderFSM, dict[str, OrderState]]:
    ...
    async def reader(eid: str) -> OrderState:
        return store[eid]

    async def writer(eid: str, s: OrderState) -> None:
        store[eid] = s

    fsm = OrderFSM(state_reader=reader, state_writer=writer)
    return fsm, store


class TestOrderFSM:
    async def test_confirm_draft(self) -> None:
        fsm, store = make_fsm(OrderState.DRAFT)
        result = await fsm.transition("ord1", OrderEvent.CONFIRM)
        assert result.success
        assert store["ord1"] == OrderState.CONFIRMED

    async def test_reject_invalid_event(self) -> None:
        fsm, store = make_fsm(OrderState.DRAFT)
        result = await fsm.transition("ord1", OrderEvent.PAYMENT_CONFIRMED)
        assert not result.success
        assert result.rejected_event == OrderEvent.PAYMENT_CONFIRMED
        assert store["ord1"] == OrderState.DRAFT  # state unchanged

    async def test_happy_path_to_cooking(self) -> None:
        fsm, store = make_fsm(OrderState.DRAFT)
        events = [
            OrderEvent.CONFIRM,
        ...
        for event in events:
            result = await fsm.transition("ord1", event)
            assert result.success, f"Failed on {event}: {result.reason}"
        assert store["ord1"] == OrderState.COOKING

    async def test_cancel_from_draft(self) -> None:
        fsm, store = make_fsm(OrderState.DRAFT)
        result = await fsm.transition("ord1", OrderEvent.CANCEL)
        assert result.success
