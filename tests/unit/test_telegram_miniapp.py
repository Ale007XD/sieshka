"""tests/unit/test_telegram_miniapp.py — Telegram Mini App staff-action
endpoints.

Mocked KitchenService/OrderService, auth dependency overridden directly to a
fixed Staff (auth mechanics themselves are covered by test_telegram_auth.py)
— this file is purely about role-gated dispatch parity with MAX/Zalo
(test_max_webhook.py/test_zalo_miniapp.py), sharing app.services.staff_dispatch.
"""
...
from app.api.routes.telegram_miniapp import (
    get_kitchen_service,
    get_order_service,
)
from app.api.routes.telegram_miniapp import router as telegram_router
...
def _staff(role: StaffRole) -> Staff:
    return Staff(id=uuid.uuid4(), name="Test", role=role, telegram_user_id=1)
...
def _client_as(role: StaffRole, mocks: _Mocks) -> AsyncClient:
    ...
    app.dependency_overrides[get_current_telegram_staff] = lambda: _staff(role)
...
class TestKitchenTransition:
    async def test_kitchen_role_allowed(self, mocks: _Mocks) -> None:
        ...
        async with _client_as(StaffRole.kitchen, mocks) as client:
            resp = await client.post(
                "/api/telegram/kitchen/ticket-1/transition", json={"event": "START_PREP"}
            )
    ...
    async def test_staff_role_allowed_for_kitchen(self, mocks: _Mocks) -> None:
        ...
        async with _client_as(StaffRole.staff, mocks) as client:
            resp = await client.post(
                "/api/telegram/kitchen/ticket-1/transition", json={"event": "MARK_READY"}
            )
        assert resp.status_code == 200
    ...
    async def test_unknown_event_denied(self, mocks: _Mocks) -> None:
        async with _client_as(StaffRole.kitchen, mocks) as client:
            resp = await client.post(
                "/api/telegram/kitchen/ticket-1/transition",
...
class TestOrderTransition:
    async def test_admin_role_allowed_cancel(self, mocks: _Mocks) -> None:
        ...
        async with _client_as(StaffRole.admin, mocks) as client:
            resp = await client.post(
                "/api/telegram/orders/order-1/transition", json={"event": "CANCEL"}
            )
        assert resp.status_code == 200
    ...
    async def test_courier_role_allowed_pickup(self, mocks: _Mocks) -> None:
        ...
        async with _client_as(StaffRole.courier, mocks) as client:
            resp = await client.post(
                "/api/telegram/orders/order-1/transition", json={"event": "PICKUP"}
            )
        assert resp.status_code == 200
