...
class MenuAgent:
    ...
    async def update_category(self, command: dict[str, Any]) -> MenuApplyResult:
        ...
        Commit/rollback at-caller convention, same as apply_menu/apply_category.
        """
        _report = ProgramValidator(PROGRAM_UPDATE_CATEGORY).validate()
        if not _report.is_valid():
            raise RuntimeError(
                f"Program '{PROGRAM_UPDATE_CATEGORY.name}' validation failed: "
                f"{_report.summary()}"
            )
...
_UPDATE_CATEGORY_TOOLS: dict[str, Callable[..., Any]] = {
    "validate_update_category_command": validate_update_category_command,
    "apply_update_category_command": apply_update_category_command,
    "report_invalid_update_category_command": report_invalid_update_category_command,
}

# Category update-phase tools that need the closure-injected session.
_UPDATE_CATEGORY_SESSION_TOOLS: frozenset[str] = frozenset({
    "validate_update_category_command",
    "apply_update_category_command",
})
