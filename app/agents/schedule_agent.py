"""
app/agents/schedule_agent.py — ScheduleAgent: two-phase agent (collect, then apply).

COLLECT phase (ALLOWED: collect input, generate command; FORBIDDEN: mutate state):
    collect_schedule() runs the LLM → validate → confirm flow and stops at a
    terminal JSON command. It writes NOTHING to Postgres.

APPLY phase (sprint_m7_agent_apply_phase_pattern):
    apply_schedule() takes a confirmed command and runs the shared apply-phase
    ...
    ScheduleWindow, through the SAME GovernedToolExecutor.check() gate as every
...
from app.policy.policy_snapshot import (
    SCHEDULE_AGENT_APPLY_POLICY_SNAPSHOT,
    SCHEDULE_AGENT_POLICY_SNAPSHOT,
)
from app.programs.schedule_agent_program import (
    PROGRAM_APPLY_SCHEDULE,
    PROGRAM_COLLECT_SCHEDULE,
)
from app.tools.schedule_agent_tools import (
    apply_schedule_command,
    collect_schedule_command,
    report_collect_failure,
    report_invalid_schedule_command,
    validate_apply_schedule_command,
    validate_schedule_command,
)
...
class ScheduleApplyResult:
    ...
    applied=False + error is None → command rejected by validate (invalid branch
        reached its terminal cleanly; nothing written).
...
class ScheduleAgent:
    """Collects schedule input, generates structured command, then applies it."""

    ALLOWED = "collect input, generate command"
    FORBIDDEN = "modify schedule state directly (collect phase only)"
    ...
    def _build_vm(self) -> _VMProtocol:
        ...
        executor = GovernedToolExecutor(policy=SCHEDULE_AGENT_POLICY_SNAPSHOT)
    ...
    def _build_apply_vm(self, session: AsyncSession) -> _VMProtocol:
        ...
        GovernedToolExecutor so schedule:write is enforced on the write step.
        """
    ...
    async def collect_schedule(
        ...
        """Process raw schedule instruction and return a structured command.

        Args:
            input_data: dict with key 'input_text' (the natural-language
                        instruction) and optional 'period'/'scope' hints.

        Returns:
            ScheduleAgentResult with success flag and structured command dict.
        """
        ...
        _report = ProgramValidator(PROGRAM_COLLECT_SCHEDULE).validate()
        if not _report.is_valid():
            raise RuntimeError(
                f"Program '{PROGRAM_COLLECT_SCHEDULE.name}' validation failed: "
                f"{_report.summary()}"
            )

        trace = await vm.run(PROGRAM_COLLECT_SCHEDULE, context=context)
        ...
        if trace.status == TraceStatus.SUCCESS:
            ...
            if fail_step:
                ...
                return ScheduleAgentResult(success=False, error=error_msg)
    ...
    async def apply_schedule(self, command: dict[str, Any]) -> ScheduleApplyResult:
        ...
        """
        _report = ProgramValidator(PROGRAM_APPLY_SCHEDULE).validate()
        if not _report.is_valid():
            raise RuntimeError(
                f"Program '{PROGRAM_APPLY_SCHEDULE.name}' validation failed: "
                f"{_report.summary()}"
            )
    ...
    async def _run_apply(
        ...
        if trace.status == TraceStatus.SUCCESS:
            ...
            logger.info("apply_schedule: command rejected (%s)", reason)
            return ScheduleApplyResult(
                applied=False, error=None, trace_id=trace_id
            )

        if session is not None:
            await session.rollback()
        error_msg = trace.error or "apply execution failed"
        logger.error("apply_schedule: apply failed — %s", error_msg)
        return ScheduleApplyResult(
            applied=False, error=error_msg, trace_id=trace_id
        )
...
_AGENT_TOOLS: dict[str, Callable[..., Any]] = {
    "validate_schedule_command": validate_schedule_command,
    "collect_schedule_command": collect_schedule_command,
    "report_collect_failure": report_collect_failure,
}

_APPLY_TOOLS: dict[str, Callable[..., Any]] = {
    "validate_apply_schedule_command": validate_apply_schedule_command,
    "apply_schedule_command": apply_schedule_command,
    "report_invalid_schedule_command": report_invalid_schedule_command,
}
